...
class EmployeeInherited(models.Model):
    ...
    def send_message_to_hr_employees_channel(self, message):
        hr_employees_group = self.env.ref('RLink_HR_Edits.group_hr_employees')
    ...
    def call_deductions_action(self):
        ...
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_deduction_action")
    ...
    def call_rotation_action(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_rotation_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
        action['domain'] = [('employee_id', '=', self.id)]
        return action
    ...
    def call_bonuses_action(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_bonus_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
        action['domain'] = [('employee_id', '=', self.id)]
        return action
    ...
    def call_salary_raises_action(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_salary_raise_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
        action['domain'] = [('employee_id', '=', self.id)]
        return action
    ...
    def call_violations_action(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_violation_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
        action['domain'] = [('employee_id', '=', self.id)]
        return action
    ...
    def call_trainings_action(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_training_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
        action['domain'] = [('employee_id', '=', self.id)]
        return action
    ...
    def call_days_off_action(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_days_off_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
        if self.days_off_id:
    ...
    def call_assessment_action(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("RLink_HR_Edits.hr_assessment_action")
        action['context'] = dict(self._context, default_employee_id=self.id, )
            ...
            if emp.user_id and emp.user_id.has_group('hr.group_hr_manager'):
                ...
                if hr_department and emp.department_id and emp.user_id:
                    if emp.department_id.id == hr_department.id and not emp.user_id.has_group(
                            'RLink_HR_Edits.group_hr_employees'):
                        hr_employees_group.write({'users': [(4, emp.user_id.id)]})
    ...
    def remove_user_from_hr_employee_channel(self, user_id):
        hr_employees_channel = self.env.ref('RLink_HR_Edits.channel_hr_employee')
